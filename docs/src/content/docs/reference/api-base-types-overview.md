...
*   **`Seq<A>`**: A contract for sequential, ordered data structures extending `Mergeable<Seq<A>>`. Defines essential operations such as `head`, `headSafe`, `tail`, `decons`, `cons`, `append`, `appendAll`, `prependAll`, `filter`, `partition` (splitting into matching and non-matching sequences), and `span` (splitting at the first non-matching element into prefix and remainder). Built-in implementations include `Sequence` (lazy segmented persistent sequence) and `SeqList` (immutable eager `java.util.List` sequence).
