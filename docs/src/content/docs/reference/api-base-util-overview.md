...
### Box
`Box<A>` is a lightweight container that holds a value lazily using a `Supplier`. It implements `Monadic`, `Value`, `Transmogrifyable`, `Unwindable`, and `Copyable`.

### Task
`Task<A>` encapsulates a lazy asynchronous computation that produces a `Result<A>`. It implements `Monadic` and `Higher1`, allowing monadic chaining (`map`, `flatMap`, `applyTo`), task combination (`zip`), custom executor binding (`executeOn`), and asynchronous execution (`runAsync()`, `runAsync(Executor)`). Any exception thrown during execution is automatically captured and wrapped into a `Result.Failure`.
...
### Executable
A functional interface extending `Fun<Executor, CompletableFuture<Result<A>>>` that represents an asynchronous operation scheduled on an `Executor`.
...
| **Utilities & Execution** | `Util`, `Zipper`, `Executable` |
