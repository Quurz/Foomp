...
*   **`Appliable<WT, A>`**: The "Applicative" contract. Allows applying functions wrapped in the same HKT context.
*   **`Monadic<WT, A>`**: A convenience interface that combines `Mappable`, `Bindable`, and `Appliable`.
*   **`Foldable<A>`**: Allows reducing a structure (like a list) into a single value. Includes `FoldableLeft` and `FoldableRight` variants.
*   **`Seq<A>`**: A contract for sequential data structures.
...
*   **`Deferrable<A>`**: Represents a value whose evaluation can be deferred. Complemented by `Eager` for immediate evaluation.
